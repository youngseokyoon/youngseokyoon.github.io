...
생성한 Bucket 의 CORS 정책위반으로 upload 가 실패함.

생성한 R2 Object Storage -> Settings 으로 이동
CORS Policy 를 클릭해서 설정을 사라펴보면 아래 처럼 localhost:3000 을 통한 GET 만 허용되어 있음.
따라서 upload 가 실패하는 것임.
```
[
  {
    "AllowedOrigins": [
